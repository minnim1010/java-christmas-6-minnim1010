...
class PromotionApplyControllerTest {
    ...
    @CsvFileSource(resources = "/프로모션_적용최소금액미만_테스트.csv", delimiter = ';', lineSeparator = "/", numLinesToSkip = 1)
    ...
    @CsvFileSource(resources = "/특별할인_테스트.csv", delimiter = ';', lineSeparator = "/", numLinesToSkip = 1)
    @ParameterizedTest(name = "day={0}, menu={1}")
    void 일요일과_크리스마스날에는_특별할인을_적용한다(int day, String menu, String expected) {
        csvFileSourceTest(day, menu, expected);
    }

    @CsvFileSource(resources = "/평일할인_테스트.csv", delimiter = ';', lineSeparator = "/", numLinesToSkip = 1)
    @ParameterizedTest(name = "day={0}, menu={1}")
    void 평일에는_평일할인을_적용한다(int day, String menu, String expected) {
        csvFileSourceTest(day, menu, expected);
    }

    @CsvFileSource(resources = "/주말할인_테스트.csv", delimiter = ';', lineSeparator = "/", numLinesToSkip = 1)
    @ParameterizedTest(name = "day={0}, menu={1}")
    void 주말에는_주말할인을_적용한다(int day, String menu, String expected) {
        csvFileSourceTest(day, menu, expected);
    }

    @CsvFileSource(resources = "/크리스마스디데이할인_테스트.csv", delimiter = ';', lineSeparator = "/", numLinesToSkip = 1)
    @ParameterizedTest(name = "day={0}, menu={1}")
    void 크리스마스당일까지_크리스마스디데이할인을_적용한다(int day, String menu, String expected) {
        csvFileSourceTest(day, menu, expected);
    }

    @CsvFileSource(resources = "/샴페인증정_테스트.csv", delimiter = ';', lineSeparator = "/", numLinesToSkip = 1)
    @ParameterizedTest(name = "day={0}, menu={1}")
    void 혜택금액이_12만원이상이라면_샴페인을_증정한다(int day, String menu, String expected) {
        csvFileSourceTest(day, menu, expected);
    }

    @CsvFileSource(resources = "/이벤트뱃지부여_테스트.csv", delimiter = ';', lineSeparator = "/", numLinesToSkip = 1)
    @ParameterizedTest(name = "day={0}, menu={1}")
